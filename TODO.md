...
- [x] `registry`, который передаётся через `SchemaFormProvider`/`SchemaForm`, теперь
      реально используется при рендере: `Renderer` берёт контролы/лейауты/действия из
      `useSchemaForm().registry`, а не из статических импортов
