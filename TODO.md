...
## Распространение readOnly/disabled по дереву

- [ ] сейчас `readOnly`/`disabled` — только пропы конкретного контрола. Нужен механизм,
      чтобы `readOnly` на форме/секции гарантированно накрывал всё дерево ниже (в референсе —
      два контекста, которые только защёлкиваются в `true`, но не сбрасываются обратно)

## Registry

- [ ] расширение/переопределение реестра контролов/лейаутов/действий вместо одного
      захардкоженного `baseRegistry`
- [ ] `registry`, который сейчас передаётся через `SchemaFormProvider`/`SchemaForm`, никем не
      используется при рендере (`Renderer` берёт контролы/лейауты/действия из статических
      импортов, а не из `registry` в контексте) — либо задействовать, либо убрать как мёртвый
      проп
